...
import { View, Text, Image, Pressable, TextInput, StyleSheet, Dimensions, Modal, ScrollView, Animated } from 'react-native';
...
const mockPosts = [
    {
        ...
        user: {
            username: 'Ashley',
            profilePic: '', // Empty profilePic to simulate missing profile picture
        },
...
const HomePage = () => {
    ...
    const handleLike = (postId) => {
        ...
        const updatedPosts = posts.map(post => {
            if (post.id === postId) {
                const isLiked = post.likes.some(like => like.userId === CURRENT_USER_ID);
                return {
                    ...post,
                    likes: isLiked 
                        ? post.likes.filter(like => like.userId !== CURRENT_USER_ID)
                        : [...post.likes, { userId: CURRENT_USER_ID }],
                };
            }
            return post;
        });
...
const handleAddComment = (postId, commentText) => {
    console.log(`Adding comment "${commentText}" to post with ID ${postId}`);
    const updatedPosts = posts.map(post => {
        if (post.id === postId) {
            return {
                ...post,
    ...
    setPosts(updatedPosts); // Update posts state with the new comment
    setCommentText(''); // Clear comment text input after posting
    ...
    const renderPost = ({ item }) => (
        <View style={styles.postContainer}>
            ...
            <View style={styles.userInfoContainer}>
                <Image source={{ uri: item.user.profilePic || 'https://via.placeholder.com/150' }} style={styles.profilePic} />
                <Text style={styles.username}>{item.user.username}</Text>
            </View>
            <Image source={{ uri: item.imageUrl }} style={styles.postImage} />
            <View style={styles.likeContainer}>
                <TouchableHeart onPress={() => handleLike(item.id)} isLiked={item.likes.some(like => like.userId === CURRENT_USER_ID)} />
            ...
            >
                <View style={styles.modalContainer}>
                    ...
                    <ScrollView style={styles.commentsScrollView}>
                        {selectedPost && selectedPost.comments.map((comment, index) => (
                            <View key={index} style={styles.commentContainer}>
                                <Text><Text style={styles.commentUsername}>{comment.user}</Text>: {comment.text}</Text>
                            </View>
                        ))}
                    </ScrollView>
                    <View style={styles.commentInputContainer}>
                        ...
                        />
                        <Pressable style={styles.addCommentButton} onPress={() => {
...
const styles = StyleSheet.create({
    container: {
        ...
        backgroundColor: '#4A2B1E', // Dark brown homepage color
        padding: 8,
    },
    postContainer: {
        marginBottom: 10,
    ...
    modalContainer: {
        ...
        backgroundColor: '#F0EAD6', // Comment section background color
    ...
    addCommentButton: {
        ...
        paddingVertical: 10,
        borderRadius: 5,
        backgroundColor: '#B76E79', // Post button color rose gold
    },
    ...
    closeModalButtonText: {
        color: '#B76E79', // Close button color rose gold
        fontWeight: 'bold',
    },
});
